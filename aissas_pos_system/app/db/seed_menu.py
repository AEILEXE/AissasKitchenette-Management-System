def seed_menu(db):
    # Legacy: destructive seed (kept for compatibility, not used by default)
    ...
    # ... rest of menu logic unchanged ...


# New: Only seed if products table is empty
def seed_menu_if_empty(db):
    """
    Seed menu only if products table is empty.
    """
    count = db.fetchone("SELECT COUNT(*) AS c FROM products;")["c"]
    if count > 0:
        print("[INFO] Products table not empty, skipping seeding.")
        return
    print("[INFO] Products table empty, seeding menu...")
    categories = [
        "Hot Coffee",
        "Iced Coffee",
        "Beef",
    ...
    print("[OK] MENU SEEDED SUCCESSFULLY (if empty)")
