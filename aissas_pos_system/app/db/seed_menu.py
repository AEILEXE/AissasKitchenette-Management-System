"""
app/db/seed_menu.py
Official Aissas Kitchenette menu seeder.

seed_menu_if_empty(db) — seeds only when products table is empty.
seed_menu(db)          — DESTRUCTIVE: clears then reseeds.

Size-based drinks (Milk Tea, Others) stored as individual products.
"""
from __future__ import annotations

_CATEGORIES = [
    "Hot Coffee", "Iced Coffee", "Beef", "Chicken Pastil",
    "Chicken Poppers Silog", "Special Sisig", "Pasta", "Premium Pasta",
    "Premium Silog", "Milk Tea", "Hot Tea", "Others",
    "Quesadilla/Sandwich", "Pizza/Burrito/Pica",
]
...
# Milk Tea Coolers — stored as "Milk Tea - <Flavor> (S/M/L)" under "Milk Tea" category
_MILK_TEA_FLAVORS = [
    "Dark Choco", "Matcha", "Okinawa", "Wintermelon", "Cookies and Cream", "Taro",
]
_MILK_TEA_SIZES = {"S": 45, "M": 55, "L": 65}

# Other Coolers — stored as "<Flavor> (S/M/L)" under "Others" category
_OTHERS_FLAVORS = [
    "Pink Lychee", "Red Strawberry", "Blue Lemonade", "Green Cucumber",
]
_OTHERS_SIZES = {"S": 35, "M": 45, "L": 55}


def _do_seed(db):
    cat_ids = {}
    for cat_name in _CATEGORIES:
        cid = db.execute_id("INSERT INTO categories (name) VALUES (?);", (cat_name,))
        cat_ids[cat_name] = cid

    def _add(cat, name, price, stock=50, active=1, low_stock=5):
        db.execute(
            "INSERT INTO products (category_id,name,price,stock,active,low_stock) VALUES (?,?,?,?,?,?);",
            (cat_ids[cat], name, float(price), stock, active, low_stock),
        )

    for cat, name, price in _FIXED_MENU:
        _add(cat, name, price)

    # Milk Tea sized drinks
    for flavor in _MILK_TEA_FLAVORS:
        for size, price in _MILK_TEA_SIZES.items():
            _add("Milk Tea", f"Milk Tea - {flavor} ({size})", price)

    # Other coolers sized drinks
    for flavor in _OTHERS_FLAVORS:
        for size, price in _OTHERS_SIZES.items():
            _add("Others", f"{flavor} ({size})", price)

    print("[OK] Menu seeded successfully.")


def seed_menu_if_empty(db):
    """Seed official menu only when products table is empty."""
